...
namespace Electracraft.Client.Website.Controllers
{
    ...
    public class TasksController : ApiController
    {
        ...
        [HttpGet]
        [ActionName("JobFiles")]
        public List<DOBase> GetJobFiles(Guid id)
        {
      ...
      )
    {
        ...
        _currentBrJob.UpdateContactRecords(newTask.JobID, newTask.ContractorID, newTask.TaskCustomerID, newTask.CreatedBy, Guid.Empty); //added guid.empty 2017.4.25 needs testing
            _currentBrSite.UpdateContactSiteRecords(newTask.SiteID, newTask.ContractorID, newTask.TaskCustomerID, newTask.CreatedBy);
        return newTask;
    }
        //2017-3-6 Jared, oli copied to me in email
        [HttpPut]
        [ActionName("UpdateStatus")]

        public DOTask updateStatus(Guid id, string status = "")
        {
            bool taskchange = false;
            DOTask task = _currentBrJob.SelectTask(id);
        ...
        [HttpPut]
        [ActionName("AddLabour")]

        public DOTaskLabour AddLabour(Guid id, Guid createdBy, Guid ContactID, string LabourDate = "", string Description = "", string StartMinute = "", string EndMinute = "", string LabourType = "", string LabourRate = "")
        {
            DOTaskLabour labour = _currentBrJob.CreateTaskLabour(id, ContactID, createdBy);
            ...
            if (LabourDate != "")
            {
                labour.LabourDate = DateTime.Parse(LabourDate);
            }
            else
            {
                labour.LabourDate = DateTime.Now;
            }
            if (StartMinute != "")
            {
                labour.StartMinute = int.Parse(StartMinute);
            }
            if (EndMinute != "")
            {
                labour.EndMinute = int.Parse(EndMinute);
            }
            //if (LabourType != "")
            //{
            //    labour.LabourType. = int.Parse(LabourType);
            //}
            //2017.3.6 Jared
            if(LabourType=="" || LabourType=="2")
            {
                labour.LabourType = TaskMaterialType.Actual;
            }
            else if(LabourType=="0")
            {
                labour.LabourType = TaskMaterialType.Quoted;
            }
            else if(LabourType=="1")
            {
                labour.LabourType = TaskMaterialType.Required;
            }
            //eob jared
            if (LabourRate != "")
            {
                labour.LabourRate = int.Parse(LabourRate);
            }



            _currentBrJob.SaveTaskLabour(labour);
